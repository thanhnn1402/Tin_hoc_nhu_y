...
if (!empty($userlogged)) {
    ...
    // Thêm địa chỉ
    if (isset($_POST['add-address-submit']) && $_POST['add-address-submit'] == 'submit') {
        ...
        if(empty($error)) {
            ...
            $sql_insert_address = "INSERT INTO `so_dia_chi`(`id_khach_hang`, `ho_ten`, `email`, `so_dien_thoai`, `tinh_tp`, `quan_huyen`, `phuong_xa`, `dia_chi_cu_the`, `trang_thai`) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";
            $stmt = $conn->prepare($sql_insert_address);
            $stmt->bind_param("isssssssi",$userlogged['id'], $fullname, $email, $phone, $city, $district, $ward, $address, $check_default);
