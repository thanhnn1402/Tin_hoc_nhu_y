...
if (!empty($adminlogged)) {
    ...
    if(!empty($id)) {
        $stmt_get_banner = $conn->prepare("SELECT * FROM banners WHERE id = ?");
		$stmt_get_banner->bind_param("s", $id);
		$stmt_get_banner->execute();
		$result_get_banner = $stmt_get_banner->get_result();
		$banner = $result_get_banner->fetch_assoc();


        //
        if (isset($_POST['edit-banner-submit']) && $_POST['edit-banner-submit'] == 'submit') {
            ...
            if(empty($error)) {
                $stmt = $conn->prepare("UPDATE `banners` SET `hinh_anh`=?,`alt`=?,`level`=? WHERE id = ?");
                $stmt->bind_param("sssi" ,$hinhanh, $alt, $level, $id);
            ...
            <div class="layout-page">
                ...
                <div class="content-wrapper">
                    ...
                    <div class="container-xxl flex-grow-1 container-p-y">
                        <h4 class="fw-bold py-3 mb-4"><span class="text-muted fw-light">Banner /</span> Chỉnh sửa banner
                        </h4>

                        <form id="form-banner-edit" action="" method="POST" enctype="multipart/form-data">
                            <div class="row bg-white">
                                    ...
                                    <div class="card mb-4 shadow-none">
                                        <div class="card-body">
                                            <div>
                                                ...
                                                <input type="text" class="form-control" id="alt" name="alt" value="<?php echo isset($banner['alt']) ? $banner['alt'] : ''; ?>" aria-describedby="defaultFormControlHelp" />
                                    ...
                                    <div class="card mb-4 shadow-none">
                                        <div class="card-body">
                                            ...
                                            <div class="grid-img mt-4">
                                                <img src="../../../storage/uploads/img/<?=$banner['hinh_anh']?>" alt="" style="width: 100%; height: auto">
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div class="col-md-12 text-end p-4">
                                    <button type="submit" name="edit-banner-submit" value="submit" class="btn btn-primary me-3">Xác nhận</button>
                                </div>
