...
if (!empty($adminlogged)) {
    ...
    if(!empty($id)) {
        $stmt_getcategory = $conn->prepare("SELECT * FROM danh_muc WHERE id = ?");
		$stmt_getcategory->bind_param("s", $id);
		$stmt_getcategory->execute();
		$result_getcategory = $stmt_getcategory->get_result();
		$banner = $result_getcategory->fetch_assoc();

        if (isset($_POST['category-edit-submit']) && $_POST['category-edit-submit'] == 'submit') {
            $tendanhmuc = isset($_POST['tendanhmuc']) ? $_POST['tendanhmuc'] : '';
            $duongdan = isset($_POST['duongdan']) ? $_POST['duongdan'] : '';
    
            if (empty($tendanhmuc) || empty($duongdan)) {
                $error['message'] = 'Vui lòng nhập trường này!';
            }
    
    
            if (empty($error)) {
                $stmt = $conn->prepare("UPDATE `danh_muc` SET `ten_danh_muc`=?,`duong_dan`=? WHERE id = ?");
                $stmt->bind_param("ssi", $tendanhmuc, $duongdan, $id);
            ...
            <div class="layout-page">
                ...
                <div class="content-wrapper">
                    ...
                    <div class="container-xxl flex-grow-1 container-p-y">
                        <h4 class="fw-bold py-3 mb-4"><span class="text-muted fw-light">Danh mục /</span> Chỉnh sửa danh mục
                        </h4>
                        ...
                        <form id="form-product-add" action="" method="POST">
                            <div class="row bg-white">
                                    ...
                                    <div class="card mb-4 shadow-none">
                                        <div class="card-body">
                                            <div>
                                                ...
                                                <input type="text" class="form-control" id="tendanhmuc" name="tendanhmuc" value="<?php echo isset($banner['ten_danh_muc']) ? $banner['ten_danh_muc'] : ''; ?>" aria-describedby="defaultFormControlHelp" />
                                    ...
                                    <div class="card mb-4 shadow-none">
                                        <div class="card-body">
                                            <div>
                                                ...
                                                <input type="text" class="form-control" id="duongdan" name="duongdan" value="<?php echo isset($banner['duong_dan']) ? $banner['duong_dan'] : ''; ?>" aria-describedby="defaultFormControlHelp" />
                                                <span id="defaultFormControlHelp" class="form-text text-danger">
                                                    <?php echo isset($error['message']) ? $error['message'] : ''; ?>
                                                </span>
                                            </div>
                                ...
                                <div class="col-md-12 text-end p-4">
                                    <button type="submit" name="category-edit-submit" value="submit" class="btn btn-primary me-3">Xác nhận</button>
                                </div>
