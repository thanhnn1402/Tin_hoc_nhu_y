...
if (!empty($adminlogged)) {
	...
	if (!empty($id)) {
		...
		if (isset($_POST['edit-user-submit']) && $_POST['edit-user-submit'] == 'submit') {
			...
			if (empty($error)) {
				// Check username
				if ($username != $user['ten_tai_khoan']) {
				...
				// Nếu không có lỗi
				if (empty($error)) {
					$stmt = $conn->prepare("UPDATE `khach_hang` SET `ten_tai_khoan`=? ,`ho_ten`=?,`email`=?,`mat_khau`=?,`gioi_tinh`='{$gender}',`ngay_sinh`='{$birthdate}',`so_dien_thoai`=?,`level`='{$level}',`avatar`='{$avatar}', `updated_at`='{$date_current}' WHERE id = ?");
					$stmt->bind_param("ssssss", $username, $fullname, $email, $password, $phone, $id);
			...
			<div class="layout-page">
				...
				<div class="content-wrapper">
					...
					<div class="container-xxl flex-grow-1 container-p-y">
						<h4 class="fw-bold py-3 mb-4"><span class="text-muted fw-light">Khách hàng /</span> Chỉnh sửa tài
							khoản</h4>
						...
						<form action="" method="POST" enctype="multipart/form-data">
							<div class="row bg-white">
									...
									<div class="card mb-4 shadow-none">
										<div class="card-body">
											<div>
												...
												<input type="text" class="form-control" id="fullname" name="fullname" value="<?php echo isset($user['ho_ten']) ? $user['ho_ten'] : ''; ?>" aria-describedby="defaultFormControlHelp" />
										...
										<div class="card-body">
											<div>
												<label for="username" class="form-label">Tên tài
													khoản</label>
												<input type="text" class="form-control" id="username" name="username" value="<?php echo isset($user['ten_tai_khoan']) ? $user['ten_tai_khoan'] : ''; ?>" aria-describedby="defaultFormControlHelp" />
												<span id="defaultFormControlHelp" class="form-text text-danger">
									...
									<div class="card mb-4 shadow-none">
										<div class="card-body">
											<div>
												...
												<input type="password" class="form-control" id="password" name="password" value="<?php echo isset($user['mat_khau']) ? $user['mat_khau'] : ''; ?>" aria-describedby="defaultFormControlHelp" />
												<span id="defaultFormControlHelp" class="form-text text-danger">
									...
									<div class="card mb-4 shadow-none">
										<div class="card-body">
											<div>
												...
												<input type="date" class="form-control" id="birthdate" name="birthdate" value="<?php echo isset($user['ngay_sinh']) ? $user['ngay_sinh'] : ''; ?>" aria-describedby="defaultFormControlHelp" />
									...
									<div class="card mb-4 shadow-none">
										<div class="card-body">
											<div>
												...
												<input type="email" class="form-control" id="email" name="email" value="<?php echo isset($user['email']) ? $user['email'] : ''; ?>" aria-describedby="defaultFormControlHelp" />
												<span id="defaultFormControlHelp" class="form-text text-danger">
										...
										<div class="card-body">
											<div>
												<label for="phone" class="form-label">Số điện
													thoại</label>
												<input type="phone" class="form-control" id="phone" name="phone" value="<?php echo isset($user['so_dien_thoai']) ? $user['so_dien_thoai'] : ''; ?>" aria-describedby="defaultFormControlHelp" />
												<span id="defaultFormControlHelp" class="form-text text-danger">
											...
											<div>
												...
												<select class="form-select" id="level" name="level" aria-label="Default select example">
													<?php foreach ($level as $key => $value) { ?>
														<option value="<?= $key ?>" <?php echo $key == $user['level'] ? 'selected' : ''; ?>><?= $key . ' - ' . $value ?></option>
													<?php } ?>
												</select>
												<span id="defaultFormControlHelp" class="form-text text-danger"></span>
											</div>
										...
										<div class="card-body">
											<div>
												<label for="create_at" class="form-label">Ngày tạo tài
													khoản</label>
												<input type="datetime-local" class="form-control" id="create_at" readonly value="<?php echo isset($user['created_at']) ? $user['created_at'] : ''; ?>" aria-describedby="defaultFormControlHelp" />
												<span id="defaultFormControlHelp" class="form-text text-danger"></span>
											</div>
										</div>
									</div>
									...
									<div class="card mb-4 shadow-none">
										<div class="card-body">
											...
											<div class="grid-img mt-4">
												<img src="../../../storage/uploads/img/<?php echo isset($user['avatar']) ? $user['avatar'] : ''; ?>" alt="">
											</div>
										</div>
									</div>
								</div>
								...
								<div class="col-md-12 text-end p-4">
									<button type="submit" name="edit-user-submit" value="submit" class="btn btn-primary me-3">Xác nhận</button>
								</div>
							</div>
						</form>
						<!-- / Content -->
